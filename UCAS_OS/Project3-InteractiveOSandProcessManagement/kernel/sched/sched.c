...
int do_kill(pid_t pid){
    ...
    pcb[i].pid==-1;
    while (!is_empty(&(pcb[i].wait_list))){
        if (list_entry(pcb[i].wait_list.next,pcb_t,list)->status != TASK_EXITED)
            do_unblock(pcb[i].wait_list.next);
        else
            delete_element(pcb[i].wait_list.next);
   
    }
    
    while (!is_empty(&(pcb[i].lock_list))){
        do_mutex_lock_release(&(list_entry(pcb[i].lock_list.next, mutex_lock_t, list)->lock));
    }
    uint64_t cpu_id;
    cpu_id = get_current_cpu_id();
...
void do_exit(void){
    ...
    while (!is_empty(&(current_running[cpu_id]->wait_list))){
        if (list_entry(current_running[cpu_id]->wait_list.next, pcb_t, list)->status!=TASK_EXITED)
            do_unblock(current_running[cpu_id]->wait_list.next);
        else
            delete_element(current_running[cpu_id]->wait_list.next);
    }

    while (!is_empty(&(current_running[cpu_id]->lock_list))){
        do_mutex_lock_release(&(list_entry(current_running[cpu_id]->lock_list.next, mutex_lock_t, list)->lock));
    }
    do_scheduler();
}
...
void do_taskset(int mask, task_info_t *task, spawn_mode_t mode){
    ...
    init_pcb_stack(pcb[i].kernel_sp, pcb[i].user_sp, task->entry_point, &pcb[i], NULL);
    pcb[i].kernel_sp=pcb[i].kernel_sp-sizeof(regs_context_t)-sizeof(switchto_context_t); 
