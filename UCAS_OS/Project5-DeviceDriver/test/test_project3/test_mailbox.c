...
void strServer(void)
{
    ...
    for (;;)
    {
        ...
        sys_move_cursor(1, 1);
        printf("[Server]: recved msg from pid:%d (blocked: %ld, correctBytes: %ld, errorBytes: %ld)",
...
void multimail(void)
{
    mailbox_t box1 = mbox_open("mailbox1");
    mailbox_t box2 = mbox_open("mailbox2");
    mailbox_t box3 = mbox_open("mailbox3");
    
    int box;
    int position = sys_getpid()%3 +1;
    int len = 0;
    int k=0;
    char strBuffer[MAX_MBOX_LENGTH]; 
    int flag;
    ...
    while(1){
        ...
        box = rand()%3 +1;
        len = 0;
        for(int j=0;j<MAX_MBOX_LENGTH;j++){
            strBuffer[j] = 0;
        }
        len = rand() % 31 + 1;
        ...
        generateRandomString(strBuffer, len);
        if(box==1){
            flag=multi_mail(box1,strBuffer,len);
            sys_move_cursor(1,position);
            if(flag==1){
                printf("[Client %d] send %d bytes to mailbox%d [%d]         \n", position,len,box,k);
            }else if(flag==2){
                printf("[Client %d] receive %d bytes from mailbox%d  [%d]     \n", position,len,box,k);
            }
        }else if(box==2){
            flag=multi_mail(box2,strBuffer,len);
            sys_move_cursor(1,position);
            if(flag==1){
                printf("[Client %d] send %d bytes to mailbox%d [%d]        \n", position,len,box,k);
            }else if(flag==2){
            ...
            flag=multi_mail(box3,strBuffer,len);
            sys_move_cursor(1,position);
            if(flag==1){
                printf("[Client %d] send %d bytes to mailbox%d [%d]         \n", position,len,box,k);
            }else if(flag==2){
